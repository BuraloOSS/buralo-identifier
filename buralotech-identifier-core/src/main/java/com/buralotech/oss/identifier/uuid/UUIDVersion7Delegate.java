 ...
 */
public class UUIDVersion7Delegate implements UUIDVersionDelegate {

    /**
     * The regular expression pattern used to validate identifiers.
     */
    private static final Pattern PATTERN = Pattern.compile("[0-9a-zA-Z_-]{8}[RSTU][0-9a-zA-Z_-][159DHLPTXaeimquy][0-9a-zA-Z_-]{10}[FVk-]");
    ...
    public boolean isValidBinary(final byte[] binary) {
        return ((binary[6] & 0xf0) == 0x70) && ((binary[8] & 0xc0) == 0x80);
    }
    ...
    public Instant toInstant(final byte[] binary) {
        return Instant.ofEpochMilli(UUIDUtil.extractTimestamp(UUIDUtil.constructUUID(UUIDType.TIME_BASED_EPOCH, binary)));
    }
