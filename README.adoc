...
* `UUIDVersion4Delegate` -- uses the standard Type 4 UUID format which is not recommended if database locality is important.
...
* Type 4 UUIDs are also supported and can be used by setting the same property as follows:
+
----
